...
(()=>{
  ...
  draw();fetch('music-waves.json?v=expansion25-1',{credentials:'omit'}).then(r=>{if(!r.ok)throw Error('wave data');return r.json()}).then(v=>{data=v;sync();}).catch(()=>{canvas.remove();});
})();
